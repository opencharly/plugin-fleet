// Package deploy is the charly plugin housing the `charly deploy …` deployment CLI. It is a
...
//
//   - command:deploy — `charly deploy add / del / show / export / import / reset / path / status /
//     from-box`, the deployment CLI. COMPILED-IN, it dispatches IN-PROC via Invoke(OpRun)
//     (runDeployCommand → kong-parse the DeployCmd tree — command.go), so the handlers run in
...
//     loaderkit.LoadHostDeployConfigViaExecutor, writes via
//     deploykit.SaveDeployConfig directly (#55 K4 config-write seam-collapse). `path`
...
// the canonical placement. The capability is advertised in Describe (NewMeta); command:deploy's
...
// `charly deploy …` CLI) in charly's own process when compiled-in.
