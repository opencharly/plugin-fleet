...
// TestDeployDelArgv_KongAccepts is the real-Kong guard for the helper every programmatic
...
//
// It lived in charly/deploy_members_test.go against a hand-copied stub of this grammar,
// because a core unit test cannot import a separate module. The grammar's owner is this
// plugin, so the guard now binds the REAL DeployDelCmd — no stub to drift out of step.
func TestDeployDelArgv_KongAccepts(t *testing.T) {
	...
	if err := parse(spec.DeployDelArgv("x")...); err != nil {
		t.Errorf("spec.DeployDelArgv produced args `charly deploy del` rejects: %v (args=%v)", err, spec.DeployDelArgv("x"))
	}
