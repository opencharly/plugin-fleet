...
// host_seams.go — the command:deploy plugin's bridge to the host. The deploy CLI handlers moved out
...
// plugin-side — reads via loaderkit.LoadHostDeployConfigViaExecutor, writes via deploykit.SaveDeployConfig
// directly (#55 K4 config-write seam-collapse; the host "deploy-config-save" leg is deleted).
// command:deploy is COMPILED-IN and dispatches
// exactly ONE `charly deploy …` invocation per process, so the reverse-channel executor is stashed
...
// setCommandContext stashes the reverse-channel executor for the duration of one `charly deploy …`
// dispatch. Called once at the top of command:deploy's Invoke(OpRun).
...
func hostDeploySeamJSON(kind string, reqAny any, replyOut any) error {
	if cmdExec == nil {
		return fmt.Errorf("deploy %s: no host reverse channel (command not compiled-in?)", kind)
	}
