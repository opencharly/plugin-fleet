...
// deploy_cmd.go — the command:deploy CLI GRAMMAR (P13). The `charly deploy …` Kong tree
...
// DeployCmd is the `charly deploy …` command group — the CLI grammar the compiled-in
// command:deploy plugin contributes to charly's Kong tree (dispatched in-proc via
// Invoke(OpRun) → dispatchDeployCLI).
type DeployCmd struct {
	...
	FromImage DeployFromBoxCmd `cmd:"" name:"from-box" help:"Source-less deploy from a built image's baked OCI labels (no charly.yml project). Pod by default; --cluster targets Kubernetes"`
...
// DeployAddCmd is the `charly deploy add <name> [<ref>]` grammar; it forwards to the
...
// DeployAddCmd's Run() (the plugin-side deploy-tree WALK) lives in walk.go (K4-C walk port).

// DeployDelCmd is the `charly deploy del <name>` grammar; Run() (walk.go) drives the
...
// derivation from the field, with `-y` as the short form — the exact contract spec.DeployDelArgv
// relies on.
type DeployDelCmd struct {
	...
	// a human `charly deploy del` never passes it and takes a byte-identical path to before.
	//
	...
	// installed 2026.223.1347 binary, `deploy del <name> --assume-yes --require-timer-unit=x` exits
...
// DeployFromBoxCmd is the `charly deploy from-box <ref> [name]` grammar. The pod path (default)
...
// DeployShowCmd is the `charly deploy show [box]` grammar (K4-C: runs entirely plugin-side —
// deploykit.LoadDeployConfig/DeployKey are already sdk-portable, no seam needed).
...
// DeployExportCmd is the `charly deploy export [boxes…]` grammar (K4-C: runs plugin-side;
...
// DeployImportCmd is the `charly deploy import <files…>` grammar (K4-C: runs plugin-side; the
// SAVE step writes plugin-side too — deploykit.SaveDeployConfig, #55 K4 config-write seam-collapse).
...
// DeployResetCmd is the `charly deploy reset [box]` grammar (K4-C: runs plugin-side; the SAVE
// step writes plugin-side too — deploykit.SaveDeployConfig, #55 K4 config-write seam-collapse).
...
// DeployStatusCmd is the `charly deploy status` grammar (K4-C: runs entirely plugin-side).
...
// DeployPathCmd is the `charly deploy path` grammar. It resolves the per-host deploy-overlay
